...
const useXREvent = (event, handler, {
  ...
  const handlerRef = React__default["default"].useRef(handler);
...
const InteractionsContext = /*#__PURE__*/React__default["default"].createContext({
  hoverState: {},
...
function InteractionManager({
  ...
  const state = fiber.useThree();
  const {
    controllers
  } = useXR();
  const [hoverState] = React__default["default"].useState(() => ({
    left: new Map(),
    right: new Map(),
    none: new Map()
  }));
  const [interactions] = React__default["default"].useState(() => ObjectsState.make());
  const addInteraction = React__default["default"].useCallback((object, eventType, handler) => {
    ObjectsState.add(interactions, object, eventType, handler);
  }, [interactions]);
  const removeInteraction = React__default["default"].useCallback((object, eventType, handler) => {
    ObjectsState.delete(interactions, object, eventType, handler);
  }, [interactions]);
  const intersect = React__default["default"].useCallback(controller => {
...
const Interactive = /*#__PURE__*/React.forwardRef((props, passedRef) => {
...
const XRContext = /*#__PURE__*/React__namespace.createContext({});

const useControllers = group => {
  const {
    gl
  } = fiber.useThree();
  const [controllers, setControllers] = React__namespace.useState([]);
...
function useHitTest(hitTestCallback) {
  ...
  const hitTestSourceRequested = React__namespace.useRef(false);
  const [hitMatrix] = React__namespace.useState(() => new three.Matrix4());
  fiber.useFrame(() => {
...
function XR({
  ...
  const [isPresenting, setIsPresenting] = React__namespace.useState(() => gl.xr.isPresenting);
  const [isHandTracking, setHandTracking] = React__namespace.useState(false);
  const [player] = React__namespace.useState(() => new three.Group());
...
function useXRButton(mode, gl, sessionInit, container) {
  const button = React__namespace.useMemo(() => {
...
function VRCanvas({
  ...
  return /*#__PURE__*/React__namespace.createElement(XRCanvas, rest, /*#__PURE__*/React__namespace.createElement(XRButton, {
    mode: "VR",
...
const useXR = () => {
  const xrValue = React__namespace.useContext(XRContext);
  const interactionsValue = React__namespace.useContext(InteractionsContext);
  const contextValue = React__namespace.useMemo(() => ({ ...xrValue,
    ...interactionsValue
...
const useXRFrame = callback => {
  ...
  const requestRef = React__namespace.useRef();
  const previousTimeRef = React__namespace.useRef();
  const loop = React__namespace.useCallback((time, xrFrame) => {
...
const useController = handedness => {
  ...
  const controller = React__namespace.useMemo(() => controllers.find(it => it.inputSource.handedness === handedness), [handedness, controllers]);
  return controller;
};
...
class XRControllerModelFactory {
  constructor(gltfLoader = null) {
    ...
    if (!this.gltfLoader) {
      this.gltfLoader = new threeStdlib.GLTFLoader();
    }
  }
...
function DefaultXRControllers({
  ...
  const [rays] = React__default["default"].useState(new Map()); // Show ray line when hovering objects
...
class HandModel extends three.Object3D {
...
exports.useXR = useXR;
exports.useXRButton = useXRButton;
